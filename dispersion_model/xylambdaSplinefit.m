...
for c = 1:n_models
    ...
    if use_crossval
        ...
        if verbose
            ...
            hold on
            scatter(...
                ones(options.grid_size(1), 1),...
                log_smooth_all(1:options.grid_size(1)),...
                'Marker', 'o', 'Color', iteration_colors(1, :), 'filled'...
            );
            for io = 2:iter
                start_io = (options.grid_size(1) + (io - 2) * options.grid_size(2) + 1);
                scatter(...
                    repmat(io, options.grid_size(1), 1),...
                    log_smooth_all(...
                        start_io:(start_io + options.grid_size(2) - 1)...
                    ),...
                    'Marker', 'o', 'Color', iteration_colors(io, :), 'filled'...
                );
            end
            plot(1:iter, log_smooth_selected, 'k-');
            ...
            scatter(...
                log_smooth_all(1:options.grid_size(1)),...
                err_samples_all(1:options.grid_size(1)),...
                'Marker', 'o', 'Color', iteration_colors(1, :), 'filled'...
            );
            for io = 2:iter
                start_io = (options.grid_size(1) + (io - 2) * options.grid_size(2) + 1);
                end_io = (start_io + options.grid_size(2) - 1);
                scatter(...
