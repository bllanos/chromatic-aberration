function f = subproblemI(M_Omega_Phi, G_xy, G_lambda_xy, J, norms, weights, nonneg)
...
% f = subproblemI(M_Omega_Phi, G_xy, G_lambda_xy, J, norms, weights, nonneg);
% b = f(Z, U, rho)
% [ b, A ] = f(Z, U, rho)
%
% ## Description
...
%   A two-element logical vector. The first element specifies whether to
...
%   latent image's spatial gradient.
%
...
%   A zero to three-element vector. The first two elements contain the
...
G_lambda_xy2 = G_lambda_xy_T * G_lambda_xy;

A_const = (M_Omega_Phi.' * M_Omega_Phi);
if ~norms(1)
    A_const = A_const + weights(1) * G_xy2;
end
if ~norms(2)
    A_const = A_const + weights(2) * G_lambda_xy2;
end
...
if nonneg
    ...
    function [ b, A ] = inner(Z, U, rho)
        b = M_Omega_Phi_J;
        if norms(1)
            b = b + (rho(1) / 2) * G_xy_T * (Z{1} - U{1});
        end
        if norms(2)
            b = b + (rho(2) / 2) * G_lambda_xy_T * (Z{2} - U{2});
        end
