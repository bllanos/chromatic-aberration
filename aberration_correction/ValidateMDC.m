...
% regularization weights, according to the minimum distance criterion
%
% ## Usage
...
% - This script runs the minimum distance criterion-based grid search
%   method for selecting regularization weights, and therefore overrides
%   the value of `solvePatchesADMMOptions.reg_options.demosaic`.
%
% ## References
...
if solvePatchesADMMOptions.reg_options.demosaic
    warning('Overriding the value of `solvePatchesADMMOptions.reg_options.demosaic`, which was `true`.');
    solvePatchesADMMOptions.reg_options.demosaic = false;
end
...
if n_active_weights < 4
    ...
    if plot_hypersurfaces && n_active_weights < 3
        ...
        for s = 1:n_iter
            ...
            path_mse_samples(s) = immse(to_spectral_matrix * in_admm.I, I_patch_gt);
        end
...
elseif plot_search_path || plot_hypersurfaces
    warning('Graphical output cannot be generated when there are more than three active regularization terms.');
end
...
save_data_filename = fullfile(output_directory, 'ValidateMDC.mat');
