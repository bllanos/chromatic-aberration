...
%     If `dispersionfun` is empty, this option is not used, and can be absent.
...
%
%   `image_bounds` is empty if `dispersionfun` is empty, as
...
%   with the same pixel dimensions as `J`.
%
...
image_sampling_J = [size(J_2D, 1), size(J_2D, 2)];
...
if nargout > 2
    ...
    if nargout > 3
        ...
        if nargout > 4
            varargout{3} = mosaic(J_full, offsetBayerPattern(target_patch, align));
