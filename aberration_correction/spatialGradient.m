...
if image_height < 2 && image_width < 2
    G_xy = spalloc(n_px_c * n_gradients, n_px_c, 0);
    G_diag = spalloc(n_px_c * n_gradients, n_px_c, 0);
    return;
end
...
if nargout > 1
    % Create filters to enforce the boundary conditions
    % - Omit the top right pixel for the up-right-gradient, and
    %   the bottom right pixel for the down-right-gradient, as these
    %   pixels get zero values from the intermediate difference formula,
    %   according to the boundary conditions (pixel replication).
    nonzero_g1 = sub2ind(...
        image_sampling,...
        repmat([repmat((1:image_height).', (image_width - 1), 1); (2:image_height).'], c, 1),...
        repmat([repelem((1:(image_width - 1)).', image_height, 1); repelem(image_width, image_height - 1, 1)], c, 1),...
        repelem((1:c).', n_px - 1, 1)...
    );
    nonzero_g2 = sub2ind(...
        image_sampling,...
        repmat([repmat((1:image_height).', (image_width - 1), 1); (1:(image_height - 1)).'], c, 1),...
        repmat([repelem((1:(image_width - 1)).', image_height, 1); repelem(image_width, image_height - 1, 1)], c, 1),...
        repelem((1:c).', n_px - 1, 1)...
    );
    ...
    col_neighbour_g1 = sub2ind(...
        image_sampling,...
        repmat([repmat([1; (1:(image_height-1)).'], (image_width - 1), 1); (1:(image_height-1)).'], c, 1),...
        repmat([repelem((2:image_width).', image_height, 1); repelem(image_width, (image_height - 1), 1)], c, 1),...
        repelem((1:c).', n_px - 1, 1)...
    );
    col_neighbour_g2 = sub2ind(...
        image_sampling,...
        repmat([repmat([(2:image_height).'; image_height], (image_width - 1), 1); (2:image_height).'], c, 1),...
        repmat([repelem((2:image_width).', image_height, 1); repelem(image_width, (image_height - 1), 1)], c, 1),...
        repelem((1:c).', n_px - 1, 1)...
    );
