...
normalize_psfs_before_combining = true;
% Normalize colour images globally, over all scene depths (true), or per
% scene depth (false)
normalize_color_images_globally = true;
...
if ~single_source && display_all_psf_each_depth
    if normalize_color_images_globally
        ...
        for j = 1:n_depths
            I_color = I_color(:, :, :, j) ./ max(max(max(I_color(:, :, :, j))));
        end
    end
    for j = 1:n_depths
        figure
