...
n_channels = 3;
image_sampling_3 = [image_sampling n_channels];
n_px = prod(image_sampling);
n_px_c = n_px * n_channels;
n_px_div4 = n_px / 4;
red_at_first_green = sub2ind(...
    image_sampling_3, rows_full{2}, cols_full{2}, repelem(1, n_px_div4, 1)...
);
red_at_second_green = sub2ind(...
    image_sampling_3, rows_full{3}, cols_full{3}, repelem(1, n_px_div4, 1)...
);
red_at_blue = sub2ind(...
    image_sampling_3, rows_full{4}, cols_full{4}, repelem(1, n_px_div4, 1)...
);
green_at_red = sub2ind(...
    image_sampling_3, rows_full{1}, cols_full{1}, repelem(2, n_px_div4, 1)...
);
green_at_blue = sub2ind(...
    image_sampling_3, rows_full{4}, cols_full{4}, repelem(2, n_px_div4, 1)...
);
blue_at_red = sub2ind(...
    image_sampling_3, rows_full{1}, cols_full{1}, repelem(3, n_px_div4, 1)...
);
blue_at_first_green = sub2ind(...
    image_sampling_3, rows_full{2}, cols_full{2}, repelem(3, n_px_div4, 1)...
);
blue_at_second_green = sub2ind(...
    image_sampling_3, rows_full{3}, cols_full{3}, repelem(3, n_px_div4, 1)...
);

% Compute the image second order derivatives
[ G_xy2, G_diag2 ] = spatialGradient2(image_sampling_3);
G_x2 = G_xy2(1:n_px_c, :);
