...
padding = paddings(1);
full_patch_size = patch_size + padding;
        ...
        if algorithm.spectral
            for pc = 1:n_patches
                mdf_weights_patches(pc, :) = selectWeightsGrid(...
                    ...
                    rho, baek2017Algorithm2Options_f, selectWeightsGridOptions,...
                ...
                mse_weights_patches(pc, :) = trainWeights(...
                    ...
                    sensor_map_resampled, bands, trainWeightsOptions,...
                    @baek2017Algorithm2, {...
            ...
            for pc = 1:n_patches
                ...
                mse_weights_patches(pc, :) = trainWeights(...
                    ...
                    sensor_map_rgb, bands_rgb, trainWeightsOptions,...
                    @baek2017Algorithm2, {...
                        rho, baek2017Algorithm2Options_f, false...
                    }, corners(i, pc, :), trainWeightsVerbose...
                );
...
for f = 1:n_admm_algorithms
    ...
    if n_active_weights == 1
        ...
        line(xlim, ylim, 'Color', 'b');
        legend('Weights', 'y = x');
        xlabel('Weight selected using the mean square error');
        ylable('Weight selected using the minimum distance criterion');
    ...
    for w = 1:n_active_weights
        ...
        xlabel(sprintf('log_{10}(Weight %d)', aw))
        title(sprintf(...
            'Agreement between MDF and MSE weights selected for %s',...
            algorithm.name...
        ));
