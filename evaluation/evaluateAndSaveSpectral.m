...
if isfield(options, 'radiance') ||...
   isfield(options, 'scanlines') ||...
   isfield(options, 'reference_patch')
    error(['The `dp.evaluation.global_spectral` evaluation options structur',...
        'e should not have the fields ''radiance'', ''scanlines'', o',...
        'r ''reference_patch'', as these fields are specific to a given image coordinate system.']);
end
...
if ~short_call
    ...
    if isfield(fg_spectral_current, 'error_map')
        savefig(...
            ...
            [name_params sprintf('_map%s.fig', metric_name)], 'compact'...
        );
    ...
    e_spectral_formatted = struct(...
        ...
        'MRAE_max', e_spectral.mrae.max,...
        'MRAE_mean', e_spectral.mrae.mean,...
        'MRAE_median', e_spectral.mrae.median,...
        'RMSE_global', e_spectral.rmse.global,...
    ...
    if isfield(e_spectral, 'radiance')
        for i = 1:length(e_spectral.radiance)
            e_spectral_formatted.(sprintf('Patch%d_MRAE', i)) = e_spectral.radiance(i).mrae;
...
if save_multi_comparisons
    ...
    if isfield(fg_spectral, 'scanlines')
        for i = 1:length(fg_spectral.scanlines)
            ...
            savefig(...
                fg_spectral.scanlines(i),...
                [...
                    I_filename,...
                    sprintf(...
                        '_evalLine%d_X%dY%dX%dY%d_%s.fig',...
                        i,...
