...
%   d-th level of dispersion in the input image.
% 
...
dataset_params.evaluation = struct(...
    ...
    'global_spectral', struct(...
        'error_map', false,...
        'mi_bands', [1, n_bands]... % 'bands_diff', [1, n_bands]...
    ),...
...
mse = zeros(n_padding, n_patch_sizes, n_noise_fractions, n_dispersion, n_criteria);
    ...
    for no = 1:n_noise_fractions
        ...
        for ps = 1:n_patch_sizes
            ...
            for pp = 1:n_padding
                ...
                time_start = tic;
                solvePatchesMultiADMMOptions.reg_options.demosaic = false;
                [...
                ...
                time_start = tic;
                solvePatchesMultiADMMOptions.reg_options.demosaic = true;
                [...
                    ~, estimated_images{criterion_index},...
                    auxiliary_images{1, criterion_index},...
                    weights_images{criterion_index},...
        ...
        if n_patch_sizes > 1 && n_padding > 1
            for f = 1:n_criteria
                surf(...
                    patch_sizes_grid, paddings_grid, mse(:, :, no, d, f),...
                    'FaceAlpha', 0.5, 'FaceColor', criteria_colors(f, :)...
                );
            end
            xlabel('Patch size');
...
if n_dispersion > 1 && n_noise_fractions > 1
    for f = 1:n_criteria
        surf(...
            dispersion_grid, noise_fractions_grid, mse_min(:, :, f),...
            'FaceAlpha', 0.5, 'FaceColor', criteria_colors(f, :)...
        );
    end
...
elseif n_noise_fractions > 1
    for f = 1:n_criteria
        plot(...
            noise_fractions_grid, mse_min(:, :, f),...
            'Color', criteria_colors(f, :), 'LineWidth', 2,...
            'Marker', 'o', 'LineStyle', '--'...
        );
    end
