...
%   weights.
% - Regularization weight selection, per image patch, using
%   image similarity (mean squared error with respect to the true image),
%   followed by image estimation by an ADMM-family algorithm, using the
%   selected per-patch regularization weights.
%
% The same ADMM-family algorithm is used for both methods:
% 'baek2017Algorithm2LowMemory()' (via 'solvePatchesMultiADMM()'),
...
%
% - 'bands': A vector containing the wavelengths at which the latent
%   images are sampled.
...
%   the pp-th patch padding size, the ps-th patch size, the no-th noise
%   fraction in the input image, and the d-th level of dispersion in the
%   input image. Time values include the time taken to generate output
%   images beyond the estimated spectral image, including visualizations of
%   selected regularization weights.
% - 'mse_time': An array with the same format and interpretation as
%   'mdc_time', containing execution timing information for image
%   estimation and regularization weights selection under the image
...
%   The minimum distance criterion is algorithm 1, and the image similarity
%   criterion is algorithm 2.
% 
...
%
% ## Notes
% - This script uses `solvePatchesMultiADMMOptions.reg_options.enabled`
...
[...
  ...
  sensor_map, bands_color, {}, solvePatchesMultiADMMOptions.sampling_options, samplingWeightsVerbose...
);
...
n_criteria = 2; % Minimum distance criterion and image similarity criterion
...
mdc_time = zeros(n_padding, n_patch_sizes, n_noise_fractions, n_dispersion);
mse_time = zeros(n_padding, n_patch_sizes, n_noise_fractions, n_dispersion);
n_patches = zeros(1, n_patch_sizes);

% Variables for plotting
mdc_color = [1, 0, 0];
mse_color = [0, 1, 0];
criteria_names = {'Minimum distance criterion', 'Mean square error'};
criteria_abbrev = {'MDC', 'MSE'};
criteria_filenames = {'mdc_', 'mse_'};
...
for d = 1:n_dispersion
    ...
    for no = 1:n_noise_fractions
        ...
        for ps = 1:n_patch_sizes
            ...
            solvePatchesMultiADMMOptions.patch_options.patch_size = patch_size;
            for pp = 1:n_padding
                padding = paddings(pp);
                solvePatchesMultiADMMOptions.patch_options.padding = padding;
                ...
                [...
                  ...
                  solvePatchesMultiADMMOptions.admm_options,...
                  solvePatchesMultiADMMOptions.reg_options,...
                  solvePatchesMultiADMMOptions.patch_options,...
                  solvePatchesMultiADMMVerbose...
                );
                mdc_time(pp, ps, no, d) = toc(mdc_time_start);
            
                % Image similarity criterion
                criterion_index = 2;
                mse_time_start = tic;
                I_in.I = I_spectral_gt;
                I_in.spectral_bands = bands;
                [...
                ...
                mse_time(pp, ps, no, d) = toc(mse_time_start);
                    ...
                    for f = 1:n_criteria
                        ...
                        for t = 1:n_steps
                            ...
                            spectral_weights_step = upsamplingWeights(...
                                bands, bands_all{t}, @sinc, solvePatchesMultiADMM.sampling_options.bands_padding...
                                );
                            [e_spectral_table_step_current, fg_spectral_step] = evaluateAndSaveSpectral(...
                                ...
                                I_spectral_gt, bands, spectral_weights_step,...
                        ...
                        writetable(...
                            ...
                            fullfile(output_directory, [name_params, criteria_filenames{f}, '_multiStep_evaluateSpectral.csv'])...
                        );
                        evaluateAndSaveSpectral(...
                            output_directory, dataset_params, [name_params_table_gt, '_multiStep'], step_name_params_tables, fg_spectral_step...
                        );
                ...
                parfor j = 1:n_j_vector
                    ...
                    for i = 1:n_i_vector
                        ...
                        for w = 1:n_active_weights
                            ...
                            if aw == 2
                                G_lambda = spectralGradient(image_sampling_f_3, solvePatchesMultiADMMOptions.admm_options.full_GLambda);
                            ...
                            if solvePatchesMultiADMMOptions.admm_options.norms(aw)
        ...
        if n_patch_sizes > 1 && n_padding > 1
            surf(...
                patch_sizes_grid, paddings_grid, mdc_time(:, :, no, d),...
                'FaceAlpha', 0.5, 'FaceColor', mdc_color...
            );
            surf(...
                patch_sizes_grid, paddings_grid, mse_time(:, :, no, d),...
        ...
        elseif n_padding > 1
            plot(...
                paddings_grid, mdc_time(:, :, no, d),...
                'Color', mdc_color, 'LineWidth', 2,...
                'Marker', 'o', 'LineStyle', '--'...
            );
            plot(...
                paddings_grid, mse_time(:, :, no, d),...
            ...
            plot(...
                patch_sizes_grid, mdc_time(:, :, no, d),...
                'Color', mdc_color, 'LineWidth', 2,...
                'Marker', 'o', 'LineStyle', '--'...
            );
            plot(...
                patch_sizes_grid, mse_time(:, :, no, d),...
                'Color', mse_color, 'LineWidth', 2,...
                'Marker', 'v', 'LineStyle', ':'...
            );
            xlabel('Patch size');
        ...
        if n_patch_sizes > 1 && n_padding > 1
            surf(...
                ...
                mdc_time(:, :, no, d) ./ ((patch_sizes_grid + 2 * paddings_grid) .^ 2),...
                'FaceAlpha', 0.5, 'FaceColor', mdc_color...
            );
            surf(...
                patch_sizes_grid, paddings_grid,...
                mse_time(:, :, no, d) ./ ((patch_sizes_grid + 2 * paddings_grid) .^ 2),...
                'FaceAlpha', 0.5, 'FaceColor', mse_color...
            );
            xlabel('Patch size');
            ylabel('Padding size');
        ...
        elseif n_padding > 1
            plot(...
                paddings_grid, mdc_time(:, :, no, d) ./ ((patch_sizes_grid + 2 * paddings_grid) .^ 2),...
                'Color', mdc_color, 'LineWidth', 2,...
                'Marker', 'o', 'LineStyle', '--'...
            );
            plot(...
                paddings_grid, mse_time(:, :, no, d) ./ ((patch_sizes_grid + 2 * paddings_grid) .^ 2),...
                'Color', mse_color, 'LineWidth', 2,...
                'Marker', 'v', 'LineStyle', ':'...
            );
            xlabel('Padding size');
            ...
            plot(...
                patch_sizes_grid, mdc_time(:, :, no, d) ./ ((patch_sizes_grid + 2 * paddings_grid) .^ 2),...
                'Color', mdc_color, 'LineWidth', 2,...
                'Marker', 'o', 'LineStyle', '--'...
            );
            plot(...
                patch_sizes_grid, mse_time(:, :, no, d) ./ ((patch_sizes_grid + 2 * paddings_grid) .^ 2),...
                'Color', mse_color, 'LineWidth', 2,...
                'Marker', 'v', 'LineStyle', ':'...
            );
            xlabel('Patch size');
        ...
        if n_patch_sizes > 1 && n_padding > 1
            surf(...
                patch_sizes_grid, paddings_grid,...
                mdc_time(:, :, no, d) ./ n_patches_rep,...
                'FaceAlpha', 0.5, 'FaceColor', mdc_color...
            );
            surf(...
                patch_sizes_grid, paddings_grid,...
                mse_time(:, :, no, d) ./ n_patches_rep,...
                'FaceAlpha', 0.5, 'FaceColor', mse_color...
            );
            xlabel('Patch size');
            ylabel('Padding size');
        ...
        elseif n_padding > 1
            plot(...
                paddings_grid, mdc_time(:, :, no, d) ./ n_patches_rep,...
                'Color', mdc_color, 'LineWidth', 2,...
                'Marker', 'o', 'LineStyle', '--'...
            );
            plot(...
                paddings_grid, mse_time(:, :, no, d) ./ n_patches_rep,...
                'Color', mse_color, 'LineWidth', 2,...
                'Marker', 'v', 'LineStyle', ':'...
            );
            xlabel('Padding size');
            ...
            plot(...
                patch_sizes_grid, mdc_time(:, :, no, d) ./ n_patches_rep,...
                'Color', mdc_color, 'LineWidth', 2,...
                'Marker', 'o', 'LineStyle', '--'...
            );
            plot(...
                patch_sizes_grid, mse_time(:, :, no, d) ./ n_patches_rep,...
                'Color', mse_color, 'LineWidth', 2,...
                'Marker', 'v', 'LineStyle', ':'...
            );
            xlabel('Patch size');
...
savefig(...
    fg,...
    fullfile(output_directory, [dataset_name, '_mse_min.fig']), 'compact'...
);
close(fg);
