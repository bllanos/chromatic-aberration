...
admm_algorithms.spectralL1L1 = struct(...
    ...
    'options', struct('norms', [true, true], 'nonneg', false)... % Custom options for baek2017Algorithm2()
);
admm_algorithms.spectralL1L1NonNeg = struct(...
    ...
    'enabled', true,...
    'spectral', true,...
    'options', struct('norms', [true, true], 'nonneg', true)...
);
admm_algorithms.spectralL1L2NonNeg = struct(...
    ...
    'enabled', true,...
    'spectral', true,...
    'options', struct('norms', [true, false], 'nonneg', true)...
);
admm_algorithms.spectralL2L1NonNeg = struct(...
    ...
    'enabled', true,...
    'spectral', true,...
    'options', struct('norms', [false, true], 'nonneg', true)...
);
admm_algorithms.spectralL2L2NonNeg = struct(...
    ...
    'enabled', true,...
    'spectral', true,...
    'options', struct('norms', [false, false], 'nonneg', true)...
);
...
admm_algorithms.colorL1L1 = struct(...
    ...
    'enabled', true,...
    'spectral', false,...
    'options', struct('norms', [true, true], 'nonneg', false)...
);
...
for i = 1:n_images
    ...
    for w = 1:n_weights
        for f = 1:length(admm_algorithm_fields)
            ...
            custom_options = fieldnames(algorithm.options);
            for f_options = 1:length(custom_options)
                baek2017Algorithm2Options.(custom_options{f_options}) =...
                    algorithm.options.(custom_options{f_options});
            end
    
            name_params = sprintf(...
                '%s_patch%dx%d_pad%d_weights%eAnd%e_',...
