...
[...
    spd_illuminant_resampled, reflectances_resampled,...
    lambda_colorChecker_resampled...
] = resampleArrays(...
    lambda_illuminant, spd_illuminant, lambda_colorChecker, reflectances,...
    'spline'...
    );
radiances = reflectances_resampled .* repmat(spd_illuminant_resampled, 1, n_patches);
% Normalization (from http://www.brucelindbloom.com/Eqn_Spect_to_XYZ.html)
[...
    spd_illuminant_resampled2, ybar_resampled,...
    lambda_xyzbar_resampled...
] = resampleArrays(...
    lambda_illuminant, spd_illuminant, lambda_xyzbar, xyzbar(:, 2),...
    'spline'...
    );
lambda_diff = diff(lambda_xyzbar_resampled);
lambda_diff = [
    lambda_diff;
    lambda_diff(end)
    ];
N = sum(spd_illuminant_resampled2 .* ybar_resampled .* lambda_diff);
radiances = radiances ./ N;

rgb = cieSpectralToRGB(...
    lambda_xyzbar, xyzbar,...
    lambda_colorChecker_resampled, radiances.',...
    illuminant_name...
    );
...
for i = 1:n_patches
    plot(...
        lambda_colorChecker_resampled, reflectances_resampled(:, i),...
